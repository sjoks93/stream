...
class LomaEngine:
    ...
    def run(self) -> Generator[TemporalMapping, None, None]:
        ...
        for ordering in self.ordering_generator():
            if self.check_state_condition(ordering):
                ...
                try:
                    ...
                    if self.check_state_mapping(temporal_mapping):
                        yielded = True
    ...
    def check_state_mapping(self, temporal_mapping):
        if self.layer.is_state:
            higher_s_mapping = temporal_mapping[self.layer.state_operand][1:]
            r_loops = self.layer.loop_relevancy_info.get_ir_layer_dims(self.layer.output_operand)
            for s_map_level in higher_s_mapping:
                for map_dim, _ in s_map_level:
                    if map_dim in r_loops:
                        return False
        return True

    def check_state_condition(self, ordering):
        if self.layer.is_state:
            found = False
